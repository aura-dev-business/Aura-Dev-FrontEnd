...
const Navigation = () => {
  ...
  const [quoteModalOpen, setQuoteModalOpen] = useState(false); // Add state for the quote modal
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ">
          <div className="flex justify-between items-center h-20 z-10">
            ...
            <div className="flex items-center z-20">
              <Link to="/" className="flex items-center group">
                <div className="relative overflow-hidden rounded-lg mr-2">
                  <img src={logo} alt="AuraDev Logo" className="w-12 h-12 object-contain transition-transform duration-300 group-hover:scale-110" />
                ...
                <div className="flex flex-col">
                  <div className="flex items-baseline gap-0">
                    <span className="text-2xl font-bold text-red-800 font-aspal">Aura</span>
                    <span className="text-2xl font-bold text-gray-800 font-aspal">Dev</span>
                  </div>
                  <span className="text-xs text-gray-500 font-medium mt-[-8px]">Build Your Digital Aura</span>
                </div>
              </Link>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-1">
                      ...
                      <AnimatePresence>
                        {activeDropdown === index && (
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: -10 }}
                            className="absolute mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-20 border border-gray-100"
                          >
              ...
              {/* Update the Get Started button to open the quote modal */}
              <button 
                onClick={openQuoteModal}
                className="ml-4 px-6 py-2 bg-red-700 hover:bg-red-800 text-white font-medium rounded-full text-sm transition-all duration-300 transform hover:scale-105 hover:shadow-md"
              >
                Get Started
              </button>
            </div>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="md:hidden absolute left-0 right-0 top-20 bg-white shadow-lg border-t border-gray-100 z-40"
            >
              <div className="py-3 px-2 max-h-[80vh] overflow-y-auto">
                        ...
                        <AnimatePresence>
                          {activeDropdown === index && (
                            <motion.div
                              ...
                              className="pl-4 bg-gray-50 border-l-2 border-red-200 mt-1 overflow-hidden"
                            >
                              {item.dropdown.map((subItem) => (
                                <Link
                                  key={subItem.path}
                ...
                <div className="px-4 py-4">
                  {/* Update the mobile Get Started button to open the quote modal */}
                  <button 
                    onClick={openQuoteModal}
                    className="w-full px-6 py-3 bg-red-700 hover:bg-red-800 text-white font-medium rounded-md text-sm transition-colors"
                  >
                    Get Started
                  </button>
                </div>
